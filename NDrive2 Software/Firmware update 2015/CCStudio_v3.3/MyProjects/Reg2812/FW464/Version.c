...
257 Dez 18	Feed-Forward Komponente in Iq_Regler basierend auf D-Anteil(Iq_ref)*Tim(PI-Regler),
2010	2010	2010
	~~~~
...
443 Sep 23	VdcBus bei 2,5 bis 5 Volt Messung ohne +0x800
	Okt 28	Erfassung Analogwerte mit flag DC1Q_MV angepasst
444 Dec 02	Regid 0x21, REGID_I_SOLLOFFSET, als Sollwert Id umfunktioniert,
			...
			Drehzahl-Rampe mit fractional integer, Abstufung für Zeiten > 4096 ms möglich,
	~~~~
2014	2014	2014
	~~~~		
444 Sep 14	- Umschalten auf einen anderen Kp Anteil bei Stromüberschwinger.
			- Kp2 ist ein prozentualer Anteil von Kp. Bedatbar zwischen 100 und 500 [%].
  	  	  	  Bsp.: Kp2=200% => Kp2=2*Kp.
			- BUG in ntcalc.c bei der Verwendung von "_IQsat(a,b)" entfernt.
			- "I_ist" und "I_ist_filt" wird nicht mehr negiert angezeigt.
		  	  (ntcalc.c -> I_components()).
			- Feed-Forward (Vorsteuerung) für Stromregler implementiert.
453 Okt 28	in obige 444 Änderung DC_1QMV keine Totzeit integriert( siehe 452),
			Ablauf DC-Bestromung ohne Wartezeit Anzeige, automatischer Ablauf schneller,
454 Okt 29	Konflikt Messung DC-Bus Bereich 2.5 bis 5 VDC, Ref150a, Uint16 behoben,
455 Eddy ???? implementieren
	~~~~
2015	2015	2015
	~~~~		

457 Jan 29	DC_1QMV, Offsetwinkel als Verstimmung nahe null
458			in between firecan
459 Sep 09	Bewertung des Messwertes Tigbt mit Ref150,
			Schwellwerte für Warnung 24600 und Abschaltung 25200 beibehalten,
	Okt 23	Zusammenfassung von weiteren Arbeitsthemen:
			Ncm reverse in torque-modus,
			Nlim < 100 % in torque-modus,
			Gaspedal für Boote mit Null in Mitte Potentiometer,
			Kompensation Sinus-, Cosinus-Geber überprüfen, ggf. korrigieren,
			mögliche Speicherbarkeit des Abgleiches prüfen, ähnlich Tacho-Abgleich,
			Datentypen von Tom übernehmen,
			prüfen RAM-Bedarf neue CAN-open (wie Deif) Implementierung emtas,
			capture_struct und para_temp wechselseitig nutzen um RAM zu sparen,
			Aufruf kernupdate() von Kommunikation und von IO-handling neu organisieren,
			Reduzierung Stromgrenze bei phasing auf max. 50 % Stellernennstrom,
			Stromgrenzen unter 2 Hz und Zeit dazu kleiner machen,
			bzw. Schaltfrequenz verringern,
			Stromgrenzen-Zeit und Stromgrenzen-IGBT-Temperatur nicht entweder-oder sondern gleichzeitig,
			bzw. bei Bamobil nicht zulassen (DSxxx sowieso ohne Temperaturmessung,
			DC1Q_MV Stellbereich bzw. Offset nahe null, dead-time compensation,
			Stromregler variable Verstärkung zwecks Verminderung Überschwinger, 
			S-Rampe von labelinghead übernehmen,
460 Nov 11	Vor ca. einem Jahr (FW444), wurde pid4_xx mit 2 weiteren Parametern versehen.
			Vorbesetzung für Voltage-Regler zu gross, keine Parametrierung per NDrive vorgesehen,
			Feldschwächung darum instabiles Verhalten,
			Vorbesetzung jetzt mit Kf=0 und Übernahme kp2 = kp,
461 Nov 13	Speichern der Abgleichwerte, kann Tacho oder SinCos-Geber sein, Offset + gain, 3-kanalig,
			handling von KernStatus.bit.SIGNMAG wegen Erweiterung auf analogtorque von Get_speed_ramp()
			nach Get_analog_command() und DefineMode()verlagert,
			Gaspedal Boote funktioniert bereits ohne Änderung, positiver Analogwert, Offset ca. -16000
			und Cutoff ca. 1000 macht torque in beide Richtungen und Nullzone,
	Dec 15	KernOptions.bit.reducelosses, bit 31, unterscheidet ob unterhalb 10 Hz Drehfeldfrequenz
			der zugelassene Max-Strom bis Dauerstrom bei 0 Hz abgebaut wird, oder ob auf 4 kHz
			pwm-Frequenz umgeschaltet wird, Hysterese 10, 10.6 Hz, Anzeige in Status, a2Hz -> a10Hz,
			Anforderung in do_Ilim() in main.c, Umschaltung mit vorhandenem Algorithmus aus interrupt.c
	~~~~
	2016
	~~~~
461	Jan	12	Umschaltung pwm erfordert neues timing, alter Zyklus erst ferig, dann aktivieren neue
			Werte auspolling in 2. Hälfte (down-count) von Zyklus, task_pol==1 mit Bedingung,
461 Jan 26	Grenzwerte DZR in torque-modus symmetrisch,
			in torque-modus bei Vorzeichenwechsel in Rampengenerator ll_xxx-Werte beachten,
462 Jan 29	pwm-Umschaltung ok, mit/ohne flag reducelosses (KernOptions regid 0x5a, bit31),
			Testversion Netzwerk, auto-Umschaltung für Dauertest zeigt bei 24 kHz pol_overrun,
462 Feb 11	ohne glitch in Drehzahl, ohne pol_overrun, int_overrun, timing komplett in int,
	Feb 18	Test mit BAMOCAR und 300 Arms Stillstand über mehrmals 15 sec. ok,
	Feb 19	Verriegelung pwm-Umschaltung mit KernMode.bit.rsvd3 auskommentiert,
	Feb 24	Reduzierung Ilim_inuse bei < 10 Hz auf CurrentAllowed_condevice bezogen, regid 0xc5,0xc4,
	Feb 25	zusätzlicher Regler pid4_Aux formal integriert, Ausgang noch nicht ausgewertet,
463 Mar 02	spezielle Implementierung Funktion konstante ZK-Spannung, Compiler switch Doppelmayr,
			Betriebsart "Digital Speed", versucht immer zu bremsen, Stomgrenze von pid4_Aux,
464	Apr 06	Messung VdcBus im Bereich 2,5 bis 5 Volt verbessert, BAMOCAR,
			Umladen ControlMode nach activControlMode auch in specials
-------------------------------------------------------------------------------------------*/
