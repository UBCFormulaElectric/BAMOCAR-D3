...
*
* HISTORY:
*  March 12, 2007: Ported to F2801x from original drv2808sci.c (D. ALTER)
*
* DESCRIPTION: Generic serial port driver for F2801x.  Supports both
...
#define GET_DEVICE(x) ((x < SCI_NUM_PORTS ) ? SciDevTab[Fd] : (PDEV_2808SCI)NULL)
/*****************************************************************************
...
}DEV_2808SCI, * PDEV_2808SCI;

DEV_2808SCI	Dev2808sci[SCI_NUM_PORTS];
 
static  PDEV_2808SCI SciDevTab[ SCI_NUM_PORTS ] = { NULL, NULL };   
...
{   
    ...
    PDEV_2808SCI pDev = GET_DEVICE( Fd );
...
SCI_DrvInit( const char * Name, SCI_INIT_OPTIONS * pInitOpts )
{
    ...
    if( DrvInit == 0 )
    {
    	...
    	//
    	memset( Dev2808sci, 0, sizeof(Dev2808sci) );
    	
    	if( SCI_NUM_PORTS == 2 )
    	{
    	    SciDevTab[0] = &Dev2808sci[0];    	
    		SciDevTab[1] = &Dev2808sci[1];
    	}
    	else
    	{
    		SciDevTab[0] = SciDevTab[1] = &Dev2808sci[0];
    	}
