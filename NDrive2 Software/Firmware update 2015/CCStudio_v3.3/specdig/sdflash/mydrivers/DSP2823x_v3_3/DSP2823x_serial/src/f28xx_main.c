...
*
* HISTORY:
*   Nov 29, 2007: Ported to F2833x from original F280x based f28xx_main.c (C. PENG)
*                 Modified PLL initialization code for F2823x.
...
*         parameters.
*     DSP2823x_GlobalVariableDefs.c
*         Included from TI release.
*     DSP2823x_usDelay.asm
*         Included from TI release.
*     Flash2823x_API_Library.lib
...
/*** Configure the PLL ***/
	...
	{													// PLL is not running in limp mode
        SysCtrlRegs.PLLSTS.bit.DIVSEL = 0;              // DIVSEL must be 0 before writing to PLLCR
		SysCtrlRegs.PLLSTS.bit.MCLKOFF = 1;				// Turn off missing clock detect before setting PLLCR

		SysCtrlRegs.PLLCR.bit.DIV = 0x000A;				// PLLx10
        ...
        SysCtrlRegs.PLLSTS.bit.DIVSEL =  2;             // /4 at reset. Change to /2.
	}
	else
...
/*** Configure the clocks ***/
	...
	SysCtrlRegs.PCLKCR3.all = 0xFFFF;       // Turn on all peripheral clocks
