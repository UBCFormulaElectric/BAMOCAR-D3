/******************************************************************************
*
* $Archive:: /TI/product/F28xxSerialFlash/src/drv2823xsci.c                    $
* $Revision:: 2                                                                $
* $Date:: 10/29/03 11:34a                                                      $
* $Author:: Tonyc                                                              $
*
* HISTORY:
*  August 29, 2008: Ported to F2823x
...
#define GET_DEVICE(x) ((x < SCI_NUM_PORTS ) ? SciDevTab[Fd] : (PDEV_2823xSCI)NULL)
/*****************************************************************************
...
}DEV_2823xSCI, * PDEV_2823xSCI;

DEV_2823xSCI	Dev2823xsci[SCI_NUM_PORTS];

static  PDEV_2823xSCI SciDevTab[ SCI_NUM_PORTS ] = { NULL, NULL };
...
AutoBaud( int Fd )
{
    ...
    PDEV_2823xSCI pDev = GET_DEVICE( Fd );
...
SCI_DrvInit( const char * Name, SCI_INIT_OPTIONS * pInitOpts )
{
    ...
    if( DrvInit == 0 )
    {
    	...
    	//
    	memset( Dev2823xsci, 0, sizeof(Dev2823xsci) );

    	if( SCI_NUM_PORTS == 2 )
    	{
    	    SciDevTab[0] = &Dev2823xsci[0];
    		SciDevTab[1] = &Dev2823xsci[1];
    	}
    	else
    	{
    		SciDevTab[0] = SciDevTab[1] = &Dev2823xsci[0];
    	}
