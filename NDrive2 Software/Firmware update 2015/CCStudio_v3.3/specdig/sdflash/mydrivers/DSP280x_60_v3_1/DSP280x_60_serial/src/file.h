...
_DECL _CODE_ACCESS int   ioctl(int fildes, int req_code, long arg );
...
_DECL _CODE_ACCESS int add_iodevice(
    char     *name,			           
    unsigned  flags,
    int      (*dopen)(const char *path, unsigned flags, int llv_fd),
    int      (*dclose)(int dev_fd),
...
_DECL  _CODE_ACCESS char * getdevname(int llv_fd );

#define _NSTREAM        20                   /* Size of stream table        */
#if defined( INCL_CIO_SUPPORT )
	// Increase _NDEVICE by 2 for SCIA and SCIB
	#define _NDEVICE        5                    /* Size of device table        */
#else
