/*H*****************************************************************************
*
* $Archive:: /TI/product/F28xxSerialFlash/src/drv2812sci.c                     $
* $Revision:: 2                                                                $
* $Date:: 10/29/03 11:34a                                                      $
* $Author:: Tonyc                                                              $
*
* DESCRIPTION: Generic serial port driver for F28xx.  Supports both
...
#define SCIA_RXINT_VECT  0x0DC1             /* PIEVECT  RXA                */
#define SCIB_RXINT_VECT  0x0DC5             /* PIEVECT  RXB                */

#define GET_DEVICE(x) ((x < SCI_NUM_PORTS ) ? SciDevTab[Fd] : (PDEV_2812SCI)NULL)
/*****************************************************************************
...
}DEV_2812SCI, * PDEV_2812SCI;

DEV_2812SCI	Dev2812sci[SCI_NUM_PORTS];
 
static  PDEV_2812SCI SciDevTab[ SCI_NUM_PORTS ] = { NULL, NULL };   
...
isrReadChar( void )
{    
    register PDEV_2812SCI    pDev; 
    register PSCI_REGS       pSci; 
    volatile unsigned      * pRxCnt;        
    Uint16                   DataVal;
                
    Uint16   PieVect  = PieCtrlRegs.PIECRTL.all;
...
{   
    ...
    PDEV_2812SCI pDev = GET_DEVICE( Fd );
...
SCI_DrvInit( const char * Name, SCI_INIT_OPTIONS * pInitOpts )
{
    ...
    if( DrvInit == 0 )
    {
    	...
    	//
    	memset( Dev2812sci, 0, sizeof(Dev2812sci) );
    	
    	if( SCI_NUM_PORTS == 2 )
    	{
    	    SciDevTab[0] = &Dev2812sci[0];    	
    		SciDevTab[1] = &Dev2812sci[1];
    	}
    	else
    	{
    		SciDevTab[0] = SciDevTab[1] = &Dev2812sci[0];
    	}
...
SCI_Open(const char * Name, unsigned Flags, int Fd )
{
    ...
    if( (Uint32)pDev->pSci == SCIA_ADDR )
    {
		PieVectTable.RXAINT = isrReadChar; 
		PieCtrlRegs.PIEIER9.all |= 0x0001;
	}
	else if ( (Uint32)pDev->pSci == SCIB_ADDR )
    {
		PieVectTable.RXBINT = isrReadChar; 
