/*H*****************************************************************************
*
* $Archive:: /TI/product/F28xxSerialFlash/src/drv2804xsci.c                    $
* $Revision:: 2                                                                $
...
*
* DESCRIPTION: Generic serial port driver for F2804x.  Supports both
...
#define GET_DEVICE(x) ((x < SCI_NUM_PORTS ) ? SciDevTab[Fd] : (PDEV_2804xSCI)NULL)
/*****************************************************************************
...
}DEV_2804xSCI, * PDEV_2804xSCI;

DEV_2804xSCI	Dev2804xsci[SCI_NUM_PORTS];
 
static  PDEV_2804xSCI SciDevTab[ SCI_NUM_PORTS ] = { NULL, NULL };   
...
{   
    ...
    PDEV_2804xSCI pDev = GET_DEVICE( Fd );
...
SCI_DrvInit( const char * Name, SCI_INIT_OPTIONS * pInitOpts )
{
    ...
    if( DrvInit == 0 )
    {
    	...
    	//
    	memset( Dev2804xsci, 0, sizeof(Dev2804xsci) );
    	
    	if( SCI_NUM_PORTS == 2 )
    	{
    	    SciDevTab[0] = &Dev2804xsci[0];    	
    		SciDevTab[1] = &Dev2804xsci[1];
    	}
    	else
    	{
    		SciDevTab[0] = SciDevTab[1] = &Dev2804xsci[0];
    	}
